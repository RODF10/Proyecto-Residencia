...
function validFile(){
    ...
    var fileExt = /(.jpg|.png|.webp|.jpeg|.gif|.ico)$/i; //Validacion de las extensiones, olo los que estan escritos

    if(!fileExt.exec(fileRuta)){//En caso de que no cumpla la expresion, mandar alerta de aviso
        console.log('Solo imagen png');
        fileCap.value = '';
        valid.image = false;
        alertaFile('error','Error al leer el archivo', 'El formato no es soportado, estas extensiones son compatibles (jpg, png, web, jpeg, gif e ico)');
        return false;
    } else {
